...
class bearing:
    ...
    def __init__(self,design_params,influence_factors,inf_factor_uncertainty):
        # attributes
        for k, v in design_params.items():        # key,values in design_param dict used to set class attributes
            setattr(self, k, v) 
        for k, v in influence_factors.items():        # key,values in influence_factors dict used to set class attributes
            setattr(self, k, v)
        for k, v in inf_factor_uncertainty.items():        # key,values in influence_factors dict used to set class attributes
            setattr(self, k, v)
    ...
    def __brg_random_variates(self):
        ...
        lamb_b_rvs = ss.lognorm.rvs(scale = self.lambda_base,s=self.lambda_base_unc,loc=0)
...
class seal:
    ...
    def __seal_random_variates(self):
        ...
        lambda_base_samp = ss.lognorm.rvs(scale = self.lambda_base,s=self.lambda_base_unc,loc=0)
        return lambda_base_samp*Cp_samp*Cq_samp*Cv_samp*Ch_samp*Cf_samp*Ct_samp*Cn_samp*Cpv_samp
...
class gearbox:
    ...
    def __gb_random_variates(self):
        ...
        return lamb_b_samp*Cgs_samp*Cgp_samp*Cga_samp*Cv_samp*Cgt_samp*Cgv_samp
...
class electric_motor:
    ...
    def __em_random_variates(self):
        # generate lognormal random variates for each inf factor and then combine for complete brg
        Csf_samp = self.Csf
        lamb_wi_samp = ss.lognorm.rvs(scale = self.lambda_windings()*self.op_time,s=self.lambda_win_base_unc,loc=0)
        lamb_bs_samp = ss.lognorm.rvs(scale = self.lambda_bs*self.op_time,s=self.lambda_bs_unc,loc=0)
        lamb_stat_samp = ss.lognorm.rvs(scale = self.lambda_stat*self.op_time,s=self.lambda_stat_unc,loc=0)
        lamb_arm_samp = ss.lognorm.rvs(scale = self.lambda_arm*self.op_time,s=self.lambda_arm_unc,loc=0)
        lamb_b_samp = ss.lognorm.rvs(scale = self.lambda_base*self.op_time,s=self.lambda_base_unc,loc=0)
        return (lamb_b_samp*Csf_samp)+lamb_wi_samp+lamb_bs_samp+lamb_stat_samp+lamb_arm_samp
